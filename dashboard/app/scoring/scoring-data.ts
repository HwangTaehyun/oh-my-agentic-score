...
const DIMENSIONS_EN: DimensionConfig[] = [
  {
    ...
    detail: [
      ...
      "",
      "How concurrent_sessions is computed:",
      "1. Gather all sessions across all projects",
      "2. For each pair of sessions, check overlap: A.start < B.end AND B.start < A.end",
      "3. Use a sweep-line to find the peak number of overlapping sessions",
      "",
  ...
  },
  {
    ...
    formula: [
      ...
      { code: "b_norm = min(log1p(b_thread_score) * 2.0 + line_bonus, 10.0)", comment: "log-scaled density score with line bonus applied" },
    ],
    fullScoreNote: "10/10: e.g. 5 sub-agents × depth 2 + 4 orchestration breadth = 14 → log1p(14)*2 = 5.4 + line bonus. Depth is the multiplier, orchestration breadth adds within-session concurrency. AI-written lines add up to +1.0 bonus (50K lines = full bonus).",
    detailTitle: "Sub-Agent Depth Detection, Orchestration Breadth & AI Lines Bonus",
    detail: [
      "Log-scaled: b_norm = min(log1p(raw_score) * 2.0 + line_bonus, 10.0)",
      ...
      "Examples: 3 agents × depth 2 + 2 orch = 8 | 5 agents × depth 2 + 4 orch = 14",
      "",
...
const DIMENSIONS_KO: DimensionConfig[] = [
  {
    ...
    detail: [
      ...
      "",
      "concurrent_sessions 계산 방법:",
      "1. 모든 프로젝트의 전체 세션을 수집",
      "2. 각 세션 쌍에 대해 시간 겹침 확인: A.start < B.end AND B.start < A.end",
      "3. Sweep-line으로 최대 동시 세션 수를 산출",
      "",
  ...
  },
  {
    ...
    formula: [
      ...
      { code: "b_norm = min(log1p(b_thread_score) * 2.0 + line_bonus, 10.0)", comment: "log 스케일 density 점수에 라인 보너스 적용" },
    ],
    fullScoreNote: "만점(10): 예) 5개 에이전트 × depth 2 + 오케스트레이션 4 = 14 → log1p(14)*2 = 5.4 + 라인 보너스. depth가 승수 역할, 오케스트레이션 폭이 세션 내 동시성을 추가. AI 작성 라인은 최대 +1.0 보너스 (5만줄 = 만점 보너스).",
    detailTitle: "Sub-Agent Depth Detection, 오케스트레이션 폭 & AI 작성 라인 보너스",
    detail: [
      "Log 스케일: b_norm = min(log1p(원시_점수) * 2.0 + line_bonus, 10.0)",
      ...
      "예: 3 × depth 2 + orch 2 = 8 | 5 × depth 2 + orch 4 = 14",
      "",
      "AI 작성 라인 보너스:",
