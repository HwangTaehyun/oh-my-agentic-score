...
export const PAGE_LABELS: Record<Lang, {
  ...
  en: {
    ...
    overallFormula: "overall = (p_thread_score + l_thread_score + min(b_thread_score + ai_line_bonus, 10) + z_thread_score) / 4",
...
const DIMENSIONS_EN: DimensionConfig[] = [
  ...
  },
  {
    ...
    formulaTitle: "Score Formula",
    formula: [
      { code: "l_thread_score = min(log1p(P90_stretch_minutes) * 2.5, 10)", comment: "P90 = 90th percentile of all autonomous stretches" },
    ],
    fullScoreNote: "10/10: ~54 minutes of P90 autonomous stretch. Derived from ln(1+54) × 2.5 ≈ 10.0",
    detailTitle: "P90 Activity-Based Measurement",
    detail: [
    ...
    referenceTable: {
      title: "Score Reference Table (P90 stretch)",
      rows: [
        { input: "0 min", score: "0.0" }, { input: "1 min", score: "1.7" },
        { input: "5 min", score: "4.5" }, { input: "10 min", score: "6.0" },
        { input: "20 min", score: "7.6" }, { input: "30 min", score: "8.6" },
        { input: "54 min", score: "10.0" },
      ],
    },
    examples: [
      {
        ...
        result: "l_thread_score = min(log1p(0) * 2.5, 10) = 0.0",
      },
      {
        title: "5-minute autonomous work (score: 4.5)",
        lines: [
          'User: "Fix the login bug"',
          "Claude works for 5 minutes: Grep → Read → Edit → Bash(test) → done.",
          "P90 autonomous stretch = 5 min",
        ],
        result: "l_thread_score = min(log1p(5) * 2.5, 10) = min(4.48, 10) = 4.48",
      },
      {
        title: "30-minute feature implementation (score: 8.6)",
        lines: [
        ...
        result: "l_thread_score = min(log1p(30) * 2.5, 10) = min(8.59, 10) = 8.59",
      },
    ],
    metrics: [
      { name: "p90_autonomous_stretch_minutes", desc: "90th percentile of autonomous stretch durations. Rewards near-longest stretches while excluding extreme outliers." },
  ...
  },
  {
    ...
    formulaTitle: "Score Formula",
    formula: [
      { code: "b_thread_score = min(total_agents, 10.0)", comment: "total agents (team + sub), capped at 10. Linear scale." },
      { code: "line_bonus = min(ai_written_lines / 50000, 1.0)", comment: "AI-written lines bonus: linear, max +1.0 at 50K lines" },
      { code: "b_norm = min(b_thread_score + line_bonus, 10.0)", comment: "density score with line bonus applied" },
    ],
    fullScoreNote: "10/10: 10 or more agents (team + sub) in a single session. Direct value — no log scale, no depth multiplier.",
    detailTitle: "Total Agent Count & AI Lines Bonus",
    detail: [
    ...
    examples: [
      {
        title: "No agents (score: 0)",
        lines: [
          "Simple session with direct tool calls only.",
          "total_agents = 0",
        ],
        result: "b_thread_score = min(0, 10) = 0",
      },
      {
        title: "3 agents (score: 3)",
        lines: [
          "Claude spawns 3 Explore agents for research.",
          "total_agents = 3",
        ],
        result: "b_thread_score = min(3, 10) = 3",
      },
      {
        title: "Team of 5 agents (score: 5)",
        lines: [
          "Claude organizes a team of 5 agents for a feature implementation.",
          "total_agents = 5",
        ],
        result: "b_thread_score = min(5, 10) = 5",
      },
      {
        title: "10+ agents (score: 10)",
        lines: [
          "Large-scale team with 10 or more agents (team + sub-agents).",
          "total_agents = 10",
        ],
        result: "b_thread_score = min(10, 10) = 10 (max!)",
      },
    ],
    metrics: [
      { name: "tool_calls_per_minute", desc: "Tool calls per minute. total_tool_calls / max(duration_minutes, 0.1)" },
      { name: "max_sub_agent_depth", desc: "Maximum sub-agent nesting depth. 0=none, 1=flat, 2+=nested (B-thread classification)." },
    ...
    scoring: [
      { range: "0", meaning: "No agents" },
      { range: "1~3", meaning: "Basic agent usage" },
      { range: "4~6", meaning: "Active agent usage (team)" },
      { range: "7~10", meaning: "Heavy agent usage (10 = max)" },
    ],
    tips: [
...
const DIMENSIONS_KO: DimensionConfig[] = [
  ...
  },
  {
    ...
    formulaTitle: "Score Formula",
    formula: [
      { code: "l_thread_score = min(log1p(P90_stretch_minutes) * 2.5, 10)", comment: "P90 = 모든 자율 구간의 90번째 백분위수" },
    ],
    fullScoreNote: "만점(10): P90 자율 구간 약 54분. ln(1+54) × 2.5 ≈ 10.0",
    detailTitle: "P90 Activity-Based Measurement (P90 활동 기반 자율 시간 측정)",
    detail: [
      "세션 내 모든 자율 구간의 90번째 백분위수(P90)를 사용합니다 (절대 최대값이 아님).",
      "P90은 거의 최장 구간을 보상하면서 극단적 이상치를 제외합니다.",
      "",
      ...
      "",
      "구간: (1) 첫 human 전, (2) human 간, (3) 마지막 human 후 — 모든 구간의 P90.",
      "",
      "Idle Gap Capping (v0.6.0+):",
    ...
    referenceTable: {
      title: "Score Reference Table (P90 자율 구간)",
      rows: [
        { input: "0분", score: "0.0" }, { input: "1분", score: "1.7" },
        { input: "5분", score: "4.5" }, { input: "10분", score: "6.0" },
        { input: "20분", score: "7.6" }, { input: "30분", score: "8.6" },
        { input: "54분", score: "10.0" },
      ],
    },
    examples: [
      {
    ...
    metrics: [
      { name: "p90_autonomous_stretch_minutes", desc: "자율 구간의 90번째 백분위수(P90). 거의 최장 구간을 보상하면서 극단적 이상치를 제외합니다." },
  ...
  {
    title: "THICKER (B-threads) — Density",
    subtitle: "세션 내 총 에이전트 수",
    color: "red", accentHex: "#FF6B35", maxScore: "10",
    formulaTitle: "Score Formula",
    formula: [
      { code: "b_thread_score = min(total_agents, 10.0)", comment: "총 에이전트 수 (team + sub), cap 10. 선형 스케일." },
      { code: "line_bonus = min(ai_written_lines / 50000, 1.0)", comment: "AI 작성 라인 보너스: 선형, 5만줄에서 만점 +1.0" },
      { code: "b_norm = min(b_thread_score + line_bonus, 10.0)", comment: "density 점수에 라인 보너스 적용" },
    ],
    fullScoreNote: "만점(10): 한 세션에서 10개 이상의 에이전트 (team + sub). 직접 값 — log 스케일 아님, depth 곱하기 없음.",
    detailTitle: "총 에이전트 수 & AI 작성 라인 보너스",
    detail: [
      "선형 스케일: b_thread_score = min(total_agents, 10). P-thread(동시 세션 수)와 같은 방식.",
      "total_agents = 세션에서 생성된 모든 에이전트 (team agent + sub-agent).",
      "",
      "depth 곱하기 없음 — 하나의 중첩 에이전트 때문에 전체 점수가 2배가 되면 안 됩니다.",
      "오케스트레이션 폭 없음 — 세션 내 동시성은 에이전트 활용의 일부일 뿐.",
      "",
      "depth는 B-thread 분류용으로 여전히 추적됩니다 (depth 2+ = B-thread),",
      "하지만 점수 계산에는 영향을 주지 않습니다.",
      "",
      "AI 작성 라인 보너스:",
    ...
    metrics: [
      ...
      { name: "max_sub_agent_depth", desc: "서브에이전트의 최대 중첩 깊이. 0=없음, 1=평면, 2+=중첩(B-thread 분류용)." },
    ...
    scoring: [
      { range: "0", meaning: "에이전트 없음" },
      { range: "1~3", meaning: "기본적인 에이전트 활용" },
      { range: "4~6", meaning: "적극적인 에이전트 활용 (팀)" },
      { range: "7~10", meaning: "대규모 에이전트 활용 (10 = 만점)" },
    ],
    examples: [
      {
        title: "에이전트 없음 (점수: 0)",
      ...
      },
      {
        title: "에이전트 3개 (점수: 3)",
        lines: [
          "Claude가 리서치용 Explore 에이전트 3개를 실행.",
          "total_agents = 3",
        ],
        result: "b_thread_score = min(3, 10) = 3",
      },
      {
        title: "팀 에이전트 5개 (점수: 5)",
        lines: [
          "Claude가 기능 구현을 위해 5개 에이전트 팀을 구성.",
          "total_agents = 5",
        ],
        result: "b_thread_score = min(5, 10) = 5",
      },
      {
        title: "10개 이상 에이전트 (점수: 10)",
        lines: [
          "대규모 팀으로 10개 이상의 에이전트 (team + sub-agent).",
          "total_agents = 10",
        ],
        result: "b_thread_score = min(10, 10) = 10 (만점!)",
      },
    ],
    tips: [
      "Team/Agent 기능을 활용하여 복잡한 작업에 여러 에이전트를 활용하세요.",
      "'팀을 구성해서 작업해줘'로 더 많은 에이전트를 병렬로 실행하세요.",
      "코드 분석 → 구현 → 테스트 → 리뷰를 각각 에이전트로 분리하세요.",
      "에이전트가 많을수록 높은 점수. 한 세션에 10개 에이전트 = 만점.",
